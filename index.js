...
let previousMonthProfitLosses = finances[0][1];
let totalChange = 0;
let greatestIncrease = { Date: '', Amount: 0 };
let greatestDecrease = { Date: '', Amount: 0 };

// Iterate through the finances array to calculate required values
finances.forEach((record, index) => {
    totalProfitLosses += record[1];

    if (index > 0) {
        let monthlyChange = record[1] - previousMonthProfitLosses;
        totalChange += monthlyChange;

        if (monthlyChange > greatestIncrease.Amount) {
            greatestIncrease = { Date: record[0], Amount: monthlyChange };
        }

        if (monthlyChange < greatestDecrease.Amount) {
            greatestDecrease = { Date: record[0], Amount: monthlyChange };
        }

        previousMonthProfitLosses = record[1];
    }
});
